...
numtests=1;                                           % Number of times to run a simulation
...
%%
% Save solution
dataFolderName = 'kumo8_Data';
fname = 'kumo6_la12_Bo1_Jan29_2025';
...
%save(fnamedotmat, 'numplugs','timeToPlug','a_values','atests','numtests','la','Bo');    %choose what variables to save
save(fnamedotmat);    % save all variables
...
% figure;
% errorbar(a_values, plugRate, plugError(:, 1), plugError(:, 2), 'o', 'LineWidth', 0.5, 'Color', 'm');    %plot with error bar
% xlabel('$a$', 'Interpreter', 'latex');
% ylabel('Plug Rate', 'Interpreter','latex');
% title('The Probability of Plug Formation Across different $a$ Values', ['la = ' num2str(la), ', Bo = ', num2str(Bo)], 'Interpreter', 'latex');
% 
% %% plot histogram of time to tipping for a given slice of 'a'
% figure;
% hist_aind = 6;         %index of 'a' slice
% timeToPlug_re = reshape(timeToPlug, numtests, []).';    %reshape timeToPlug so each row is the time to plug for a give 'a'
% histogram(timeToPlug_re(hist_aind, :), 40);                    %plot histogram
...
%     fig = figHandles(i);
%     filename = sprintf('figure_%d.png', i);  % or use get(fig, 'Name') if set
%     saveas(fig, filename);
% end
% 
